 ...
 */
    ...
    public function _setupAuth() {
        ...
        $this->Auth->loginRedirect = array('admin' => true, 'controller' => 'admin_users', 'action' => 'index');
        $this->Auth->loginAction = array('admin' => true, 'controller' => 'admin_users', 'action' => 'login');
 ...
 */
    public function login() {
        if($this->request->is('post')) {
			$this->request->data['User'] =  $this->request->data['AdminUser']; 
            if($this->Auth->login()) {
                if ($this->Auth->user()) {
                    $this->AdminUser->updateLastActivity($this->Auth->user('id'), 'last_login');
 ...
 */
    public function dashboard() {
        $user = $this->AdminUser->read(null, $this->Auth->user('id'));
        $this->set('user', $user);
    }
 ...
 */
    public function view($slug = null) {
        try {
            $this->set('user', $this->AdminUser->view($slug));
        } catch (Exception $e) {
 ...
 */
    public function add() {
        ...
        if (!empty($this->request->data)) {
            $user = $this->AdminUser->register($this->request->data);
            if ($user !== false) {
                $this->_sendVerificationEmail($this->AdminUser->data);
 ...
 */
    public function verify($token = null) {
        try {
            $this->AdminUser->verifyEmail($token);
 ...
 */
    public function request_new_password($token = null) {
        ...
        $data = $this->AdminUser->validateToken($token, true);
        ...
        if ($this->AdminUser->save($data, array('validate' => false))) {
 ...
 */
    public function change_password() {
        if ($this->request->is('post')) {
            ...
            if ($this->AdminUser->changePassword($this->request->data)) {
                $this->flashMessage(__d('users', 'Password changed.'), 'alert-info', '/admin');
            }
        }
    }
 ...
 */
    ...
    public function admin_view($id = null) {
        ...
        $this->set('user', $this->AdminUser->read(null, $id));
    }

    public function admin_add() {
		if($this->request->is('post')){
			if ($this->AdminUser->add($this->request->data)) {
    ...
    public function admin_edit($userId = null) {
        $this->AdminUser->recursive = 0;
        try {
            $result = $this->AdminUser->edit($userId, $this->request->data);
            if ($result === true) {
                $this->flashMessage(__d('users', 'The User has been saved'), 'alert-success', array('action' => 'index'));
            } else {
        ...
        if (empty($this->request->data)) {
            $this->request->data = $this->AdminUser->read(null, $userId);
        }
    }

    public function admin_delete($userId = null) {
        $this->AdminUser->recursive = 0;
        if ($this->AdminUser->delete($userId)) {
 ...
 */
    protected function _sendPasswordReset($admin = null, $options = array()) {
        ...
        if (!empty($this->request->data)) {
            $user = $this->AdminUser->passwordReset($this->request->data);
            ...
            if (!empty($user)) {
                ...
                $Email->to($user[$this->modelClass]['email'])
                    ...
                    ->viewVars(array(
                        ...
                        'token' => $this->AdminUser->data[$this->modelClass]['password_token']))
                    ->send();
					print_r('test');die;
                if ($admin) {
 ...
 */
    protected function _resetPassword($token) {
        $user = $this->AdminUser->checkPasswordToken($token);
        if (empty($user)) {
            $this->flashMessage(__d('users', 'Invalid password reset token, try again.'), 'alert-error', array('action' => 'reset_password'));
        }

        if (!empty($this->request->data) && $this->AdminUser->resetPassword(Set::merge($user, $this->request->data))) {
 ...
 */
    protected function _setCookie($options = array(), $cookieKey = 'AdminUser') {
